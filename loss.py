...
def ot_loss_2(proto_s, feat_tu_w, feat_tu_s, args):
    bs = feat_tu_s.shape[0]
    with torch.no_grad():
        M_st_weak = 1 - pairwise_cosine_sim(proto_s, feat_tu_w)  # postive distance A和原型的距离
    gamma_st_weak = ot_mapping(M_st_weak.data.cpu().numpy().astype(np.float64)) # A和原型的OT
    score_ot, pred_ot = gamma_st_weak.t().max(dim=1)
    Lm = center_loss_cls(proto_s, feat_tu_s, pred_ot, num_classes=args.class_num) # B和原型的OT
    return Lm
